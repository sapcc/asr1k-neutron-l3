...
class ASR1KFWaaSDriver(driver_api.FirewallDriverDB, driver_api.FirewallDriverRPCMixin):

    SUPPORTED_L3_PORTS = set([nl_constants.DEVICE_OWNER_ROUTER_GW, nl_constants.DEVICE_OWNER_ROUTER_INTF])
    ...
    def notify_asr1k_agent(self, entity, id, context, router_ids=None, fwg_policy_ids=None):
        ...
        if isinstance(router_ids, list):
            ...
            LOG.debug("{} {} has port/policy associated that belongs to routers {}."
                      " Notifying asr1k agent.".format(entity, id, router_ids))
    ...
    def create_firewall_group_postcommit(self, context, firewall_group):
        ...
        if firewall_group['ingress_firewall_policy_id'] is None and firewall_group['egress_firewall_policy_id'] is None:
            LOG.debug("firewall_group {} has no policies attached. Doing nothing".format(firewall_group['id']))
            return
        if not firewall_group['ports']:
            LOG.debug("firewall_group {} has no ports attached. Doing nothing".format(firewall_group['id']))
            return
        ports = firewall_group['ports']
        router_ids = self.asr1k_db.get_router_id_by_ports(context, ports)
        self.notify_asr1k_agent('firewall_group', firewall_group['id'], context, router_ids=router_ids)
    ...
    def update_firewall_group_postcommit(self, context, old_firewall_group, new_firewall_group):
        ...
        if len(updated_ports) == 0:
            return
        router_ids = self.asr1k_db.get_router_id_by_ports(context, updated_ports)
        self.notify_asr1k_agent('firewall_group', new_firewall_group['id'], context, router_ids=router_ids)
    ...
    def delete_firewall_group_postcommit(self, context, firewall_group):
        ...
        if not firewall_group['ports']:
            LOG.debug("firewall_group {} has no ports attached. Doing nothing".format(firewall_group['id']))
            return
        router_ids = self.asr1k_db.get_router_id_by_ports(context, firewall_group['ports'])
        self.notify_asr1k_agent('firewall_group', firewall_group['id'], context, router_ids=router_ids)
    ...
    def update_firewall_policy_postcommit(self, context, old_firewall_policy, new_firewall_policy):
        if old_firewall_policy['firewall_rules'] != new_firewall_policy['firewall_rules']:
            ...
            self.notify_asr1k_agent('firewall_policy', fwg_id, context, fwg_policy_ids=[fwg_id])
    ...
    def insert_rule_postcommit(self, context, policy_id, rule_info):
        self.notify_asr1k_agent('firewall_policy', policy_id, context, fwg_policy_ids=[policy_id])

...
class DBPlugin(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def get_router_id_by_ports(self, context, ports):
        query = context.session.query(models_v2.Port.device_id) \
            .filter(models_v2.Port.id.in_(ports)) \
            .filter(models_v2.Port.device_owner.in_([
                *n_constants.ROUTER_INTERFACE_OWNERS, n_constants.DEVICE_OWNER_ROUTER_GW])) \
            .distinct()    
        return [x.device_id for x in query.all()]
    ...
    def get_policies_by_router_id(self, context, router_id):
        query = context.session.query(fwaas.FirewallPolicy) \
            ...
            .join(models_v2.Port) \
            .filter(fwaas.FirewallGroup.admin_state_up is True) \
            .filter(models_v2.Port.device_owner.in_([
                *n_constants.ROUTER_INTERFACE_OWNERS, n_constants.DEVICE_OWNER_ROUTER_GW])) \
            .filter(models_v2.Port.device_id == router_id) \
            .distinct()
        return query.all()
    ...
    def get_routers_with_policy(self, context, host=None, policy_id=None):
        query = context.session.query(agent_model.Agent.host, models_v2.Port.device_id) \
            ...
            .join(models_v2.Port, l3agent_models.RouterL3AgentBinding.router_id == models_v2.Port.device_id) \
            .join(fwaas.FirewallGroupPortAssociation,
                  fwaas.FirewallGroupPortAssociation.port_id == models_v2.Port.id) \
            .join(fwaas.FirewallGroup,
                  fwaas.FirewallGroup.id == fwaas.FirewallGroupPortAssociation.firewall_group_id)
        if host:
            query = query.filter(agent_model.Agent.host == host)
        if policy_id:
            query = query.filter(or_(
                        fwaas.FirewallGroup.egress_firewall_policy_id == policy_id,
                        fwaas.FirewallGroup.ingress_firewall_policy_id == policy_id))
        else:
            query = query.filter(or_(

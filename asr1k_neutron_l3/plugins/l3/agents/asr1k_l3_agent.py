        ...
        if cfg.CONF.asr1k.init_mode:
            ...
            for context in self.asr1k_pair.contexts:
                if not context.wait_alive():
                    LOG.error(f"Connection to {context.host} not alive, still continuing startup")
                else:
                    LOG.debug(f"Connection to {context.host} alive")
        ...
        try:
            if not self.pause_process:
                for rp, update in self._queue.each_update_to_next_router():
                    if update.action != l3_agent.DELETE_ROUTER and update.id in self._deleted_routers:

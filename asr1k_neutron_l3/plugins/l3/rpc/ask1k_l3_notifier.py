...
class ASR1KAgentNotifyAPI(l3_rpc_agent_api.L3AgentNotifyAPI):
    ...
    def router_sync(self, context, router_id):
        if router_id:
            return self._agent_rpc(context, 'router_sync', router_id)

    @log_helpers.log_method_call
    def router_validate(self, context, router_id):
        if router_id:
            return self._agent_rpc(context, 'router_validate', router_id)

    @log_helpers.log_method_call
    def router_teardown(self, context, router_id):
        if router_id:
            return self._agent_rpc(context, 'router_teardown', router_id)

    @log_helpers.log_method_call
    def interface_statistics(self, context, router_id):
        if router_id:
            return self._agent_rpc(context, 'interface_statistics', router_id)




    @log_helpers.log_method_call
    def _agent_rpc(self, context, method, router_id):
        ...
        cctxt = self.client.prepare(topic=topics.L3_AGENT,
                                    server=host,
                                    version='1.1')
        return cctxt.call(context, method, router_id=router_id)

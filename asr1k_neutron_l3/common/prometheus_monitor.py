...
class PrometheusMonitor(object):
    ...
    def __setup__(self, host=None, namespace=None, type=L3):
        ...
        self.type = type
        self.host = host
        self._requeue = Counter('requeue', 'Number of operaton requeues',
                                DETAIL_LABELS, namespace=self.namespace)
        self._ssh_banner_errors = Counter('ssh_banner_errors', 'Number of ssh banner errors',
                                          DETAIL_LABELS, namespace=self.namespace)
        self._inconsistency_errors = Counter('inconsistency_errors', 'Number of device inconsistency_errors',
                                             DETAIL_LABELS, namespace=self.namespace)
        self._internal_errors = Counter('internal_errors', 'Number of device API internal errors',
                                        DETAIL_LABELS, namespace=self.namespace)
        self._config_locks = Counter('config_locks', 'Number of device config_locks',
                                     DETAIL_LABELS, namespace=self.namespace,)
        self._nc_ssh_errors = Counter('nc_ssh_errors', 'Number of netconf-yang SSH errors',
                                      DETAIL_LABELS, namespace=self.namespace)
        ...
        self._connection_pool_exhausted = Counter('connection_pool_exhausted', 'Connnection pool  exhausted',
                                                  CONNECTION_POOL_LABELS, namespace=self.namespace)

        self._yang_operation_duration = Histogram("yang_operation_duration", "Individual entity operation",
                                                  DETAIL_LABELS, namespace=self.namespace, buckets=OPERATION_BUCKETS)

        self._l3_orphan_count = Gauge('l3_orphan_count', 'Number of L3 orphans found on device',
        ...
        if self.type == L3:
            ...
            self._router_update_duration = Histogram("router_update_duration", "Router update duration in seconds",
                                                     BASIC_LABELS, namespace=self.namespace, buckets=ACTION_BUCKETS)
            self._router_delete_duration = Histogram("router_delete_duration", "Router delete duration in seconds",
                                                     BASIC_LABELS, namespace=self.namespace, buckets=ACTION_BUCKETS)
            self._config_copy_duration = Histogram("config_copy_duration",
                                                   "Running to starup config copy duration in seconds",
                                                   DETAIL_LABELS, namespace=namespace, buckets=ACTION_BUCKETS)
            self._config_copy_errors = Counter('config_copy_errors', 'Number of config copy errors',
                                               DETAIL_LABELS, namespace=self.namespace)
            self._routers = Gauge('routers', 'Number of managed routers', STATS_LABELS, namespace=self.namespace)
            self._interfaces = Gauge('interfaces', 'Number of managed interfaces',
            ...
            self._floating_ips = Gauge('floating_ips', 'Number of managed floating_ips',
                                       STATS_LABELS, namespace=self.namespace)
            self.fwaas_clean_duration = Histogram("fwaas_cleaner_duration", "FWaaS cleaner runtime in seconds",
            ...
            self._port_create_duration = Histogram("port_create_duration", "Port create duration in seconds",
                                                   BASIC_LABELS, namespace=self.namespace, buckets=ACTION_BUCKETS)
            self._port_update_duration = Histogram("port_update_duration", "Port update duration in seconds",
                                                   BASIC_LABELS, namespace=self.namespace, buckets=ACTION_BUCKETS)
            self._port_delete_duration = Histogram("port_delete_duration", "Port delete duration in seconds",
                                                   BASIC_LABELS, namespace=self.namespace, buckets=ACTION_BUCKETS)

...
class Port:
    ...
    def __init__(self, asr_pair, port_info):
        self.contexts = asr_pair.contexts
    ...
    def create(self, callback=None):
        ...
        for context in self.contexts:
            ext_interface = l2_interface.ExternalInterface(context, port_channel=self.ext_portchannel,
                                                           id=segmentation_id, description=network_id)
            lb_ext_interface = l2_interface.LoopbackExternalInterface(context, port_channel=self.lb_ext_portchannel,
                                                                      id=service_instance, description=port_id,
                                                                      dot1q=segmentation_id, second_dot1q=second_dot1q)
            lb_int_interface = l2_interface.LoopbackInternalInterface(context, port_channel=self.lb_int_portchannel,
                                                                      id=service_instance, description=port_id,
    ...
    def delete(self, callback=None):
        ...
        port_id = self.port_info.get('port_id')
        segmentation_id = self.port_info.get('segmentation_id')
        service_instance = self.port_info.get('service_instance')
